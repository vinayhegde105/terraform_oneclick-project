...
resource "azurerm_network_security_rule" "vthnsg" {
  name                        = "test123"
  priority                    = 100
  direction                   = "Outbound"
  access                      = "Allow"
...
resource "azurerm_network_interface_security_group_association" "vthanisga" {
    network_interface_id      = azurerm_network_interface.myvmnic.id
    network_security_group_id = azurerm_network_security_group.vth.id
}
